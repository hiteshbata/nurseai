...
const { hasUrlError, hasRecoveryParams, isInviteFlow } = mod.exports
...
// ── isInviteFlow: post-password-set routing branch ────────────────────────

test('type=invite in the query string is an invite flow', () => {
  const [search] = params('type=invite')
  assert.equal(isInviteFlow(search), true)
})

test('type=recovery in the query string is NOT an invite flow', () => {
  const [search] = params('type=recovery')
  assert.equal(isInviteFlow(search), false)
})

test('a bare PKCE code (recovery) is NOT an invite flow', () => {
  const [search] = params('code=abc123')
  assert.equal(isInviteFlow(search), false)
})

test('no type param is NOT an invite flow', () => {
  const [search] = params('foo=bar')
  assert.equal(isInviteFlow(search), false)
})
