...
export default function ListeningMistakesPage() {
  ...
  const revealExplanation = async (questionId: number) => {
    setExplaining(questionId)
    try {
      const res = await api.get(`/listening/questions/${questionId}/explanation`)
      setMistakes((prev) =>
        prev.map((m) => (m.questionId === questionId ? { ...m, explanation: res.data.explanation } : m))
      )
    } catch (error) {
      if (!isUpgradeRequiredError(error)) toast.error('Failed to load explanation — try again')
    } finally {
      setExplaining(null)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <Link href="/practice/listening" className="text-sm text-gray-500 hover:text-gray-700 mb-4 inline-block">
          ← Back to listening
        </Link>
        ...
        {upgradeRequired ? (
          <UpgradeRequired
            ...
            message="Upgrade your plan to unlock your listening mistake history."
          />
        ) : mistakes.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-lg shadow mt-8">
            ...
            <p className="text-muted-foreground">Take a listening test and anything you get wrong will show up here for review.</p>
          </div>
        ) : (
          <div className="space-y-6 mt-8">
            {mistakes.map((m) => (
              <Card key={m.questionId} className="p-6">
                {m.section_title && (
                  <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{m.section_title}</span>
                )}
                ...
                <div className="mt-4">
                  {m.explanation ? (
                    <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3">{m.explanation}</p>
                  ) : (
                    <Button
                      variant="outline"
                      onClick={() => revealExplanation(m.questionId)}
                      disabled={explaining === m.questionId}
                    >
                      {explaining === m.questionId ? 'Thinking...' : 'Why?'}
                    </Button>
                  )}
                </div>
              </Card>
