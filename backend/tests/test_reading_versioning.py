...
def test_version_detail_returns_exact_frozen_snapshot(monkeypatch):
    ...
    # published_by_display (RC4.3.2.1) is a computed field layered on top of
...
# ── RC4.3.2.1 -- publisher identity (published_by_display) ─────────────
# published_by is a raw auth.users UUID; published_by_display resolves it
# via the public.users mirror table (same lookup admin.py's User 360 already
# uses). _seeded_fake() never seeds "users", so admin-1 has no mirror row by
...
def test_version_list_shows_known_publisher_display_name(monkeypatch):
    ...
    v = reading_router.list_reading_test_versions(1, _admin=_ADMIN)["versions"][0]
...
def test_version_list_null_publisher_has_no_display(monkeypatch):
    ...
    monkeypatch.setattr(reading_router, "get_supabase", lambda: fake)
    reading_router.set_test_active(1, SetActiveRequest(is_active=True), admin=_ADMIN)
    fake.tables["reading_test_versions"][0]["published_by"] = None  # e.g. a system backfill
...
def test_version_list_is_current_unaffected_by_publisher_lookup(monkeypatch):
    ...
    monkeypatch.setattr(reading_router, "get_supabase", lambda: fake)
    reading_router.set_test_active(1, SetActiveRequest(is_active=True), admin=_ADMIN)   # v1
    reading_router.set_test_active(1, SetActiveRequest(is_active=False), admin=_ADMIN)
    reading_router.set_test_active(1, SetActiveRequest(is_active=True), admin=_ADMIN)   # v2

    versions = reading_router.list_reading_test_versions(1, _admin=_ADMIN)["versions"]
...
def test_versions_endpoints_still_read_only_with_publisher_lookup(monkeypatch):
    ...
    monkeypatch.setattr(reading_router, "get_supabase", lambda: fake)
    reading_router.set_test_active(1, SetActiveRequest(is_active=True), admin=_ADMIN)
    before_versions = json.loads(json.dumps(fake.tables["reading_test_versions"]))
    before_users = json.loads(json.dumps(fake.tables["users"]))

    reading_router.list_reading_test_versions(1, _admin=_ADMIN)
    reading_router.get_reading_test_version(1, fake.tables["reading_test_versions"][0]["id"], _admin=_ADMIN)

    assert fake.tables["reading_test_versions"] == before_versions
